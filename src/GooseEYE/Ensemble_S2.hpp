...
void Ensemble::S2(const ArrS &f, const ArrS &g)
{
  // optionally use masked implementation
  if ( mPad.size() > 0 ) return S2(f, g, ArrS::Zero(f.shape()), ArrS::Zero(g.shape()));
  ...
  std::vector<int> shape = this->shape(F, "f");

  // allocate 'output'
  cppmat::array<double> out = cppmat::array<double>::Zero(mData.shape());
...
void Ensemble::S2(const ArrS &f, const ArrS &g, const ArrS &fmask, const ArrS &gmask)
{
  // checks
  if ( f.rank()  != mData.rank()  ) throw std::length_error("S2: rank inconsistent");
  ...
  // allocate 'output'
  cppmat::array<double> out  = cppmat::array<double>::Zero(mData.shape());
  cppmat::array<double> norm = cppmat::array<double>::Zero(mData.shape());
