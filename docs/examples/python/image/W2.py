...
(WIgr,norm) = gimage.W2(W,Igr,(101,101),mask=W,periodic=True,zeropad=False)
...
plt.xlabel(r'$\Delta x$')
plt.ylabel(r'$\Delta y$')
plt.title(r'$\mathcal{W} \star (\mathcal{M} \mathcal{I})$')
div  = make_axes_locatable(ax)
cax  = div.append_axes("right", size="5%", pad=0.1)
cbar = plt.colorbar(im,cax=cax)
cbar.set_ticks([0,1])
...
plt.xlabel(r'$\Delta x$')
plt.ylabel(r'$\Delta y$')
plt.title(r'$\mathcal{W} \star (\mathcal{M} \mathcal{I}) - \langle \mathcal{I} \rangle$')
div  = make_axes_locatable(ax)
cax  = div.append_axes("right", size="5%", pad=0.1)
cbar = plt.colorbar(im,cax=cax)
