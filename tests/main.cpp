...
SECTION("relabel_map")
{
    ...
    REQUIRE(xt::all(xt::equal(GooseEYE::relabel_map(b, a), xt::xtensor<size_t,1>{0, 0, 0, 1, 2})));
}
...
SECTION("path")
{
    ...
    REQUIRE(xt::all(xt::equal(GooseEYE::path({0, 0}, {0, 2}, GooseEYE::path_mode::Bresenham), path)));
    REQUIRE(xt::all(xt::equal(GooseEYE::path({0, 0}, {0, 2}, GooseEYE::path_mode::actual), path)));
    REQUIRE(xt::all(xt::equal(GooseEYE::path({0, 0}, {0, 2}, GooseEYE::path_mode::full), path)));
}
